package com.kevin.quick.common.dao.dao.impl;

import com.kevin.quick.common.core.exception.BizException;
import com.kevin.quick.common.dao.bean.BaseEntity;
import com.kevin.quick.common.dao.bean.Page;
import com.kevin.quick.common.dao.bean.PageParam;
import com.kevin.quick.common.dao.dao.BaseDao;
...
public abstract class BaseDaoImpl<T extends BaseEntity> implements BaseDao<T> {
  ...
  /**
   * 注入SqlSessionTemplate实例(要求Spring中进行SqlSessionTemplate的配置). 可以调用sessionTemplate完成数据库操作.
   */
  @Autowired
  ...
  public int insert(T entity) {
    ...
    if (result <= 0) {
      throw BizException.DB_INSERT_RESULT_0
          .newInstance("数据库操作,insert返回0.{%s}", getStatement(SQL_INSERT));
    }
    return result;
  }
  ...
  public int insert(List<T> list) {
    ...
    if (result <= 0) {
      throw BizException.DB_INSERT_RESULT_0
          .newInstance("数据库操作,batchInsert返回0.{%s}", getStatement(SQL_BATCH_INSERT));
    }
    return result;
  }
  ...
  public int update(T entity) {
    ...
    if (result <= 0) {
      throw BizException.DB_UPDATE_RESULT_0
          .newInstance("数据库操作,updateByPrimaryKey返回0.{%s}", getStatement(SQL_UPDATE_BY_ID));
    }
    return result;
  }
  ...
  public int update(List<T> list) {
    ...
    if (result <= 0) {
      throw BizException.DB_UPDATE_RESULT_0
          .newInstance("数据库操作,batchUpdateByIds返回0.{%s}", getStatement(SQL_BATCH_UPDATE_BY_IDS));
    }
    return result;
  }
  ...
  public Page listPage(PageParam pageParam, Map<String, Object> paramMap) {
    ...
    int currentPage = Page
        .checkCurrentPage(totalCount.intValue(), pageParam.getNumPerPage(), pageParam.getPageNum());
    pageParam.setPageNum(currentPage); // 为当前页重新设值
    // 校验页面输入的每页记录数numPerPage是否合法
    int numPerPage = Page.checkNumPerPage(pageParam.getNumPerPage()); // 校验每页记录数
    ...
    if (isCount != null && "1".equals(isCount.toString())) {
      ...
      return new Page(pageParam.getPageNum(), pageParam.getNumPerPage(), totalCount.intValue(),
          list, countResultMap);
    } else {
      // 构造分页对象
      return new Page(pageParam.getPageNum(), pageParam.getNumPerPage(), totalCount.intValue(),
